...
parser = argparse.ArgumentParser(description='Stage-2 AdaIR refinement for Histoformer Dehaze')

parser.add_argument('-learning_rate', help='Set the learning rate', default=1e-4, type=float)
parser.add_argument('-train_batch_size', help='Set the training batch size', default=2, type=int)
parser.add_argument('-train_epoch', help='Set the training epoch', default=200, type=int)

parser.add_argument('--dataset_root', type=str, required=True,
                    help='Path to dataset root, e.g. /project/6050328/heydam3/paper/dataset/Dense-Haze')
parser.add_argument('--stage1_ckpt', type=str, required=True,
                    help='Path to the pretrained stage-1 checkpoint (.pkl)')
...
parser.add_argument('--model_save_dir', type=str, default='Stage2-Checkpoints')
parser.add_argument('--log_txt', type=str, default='stage2_train_log.txt',
                    help='Training log txt filename')
parser.add_argument('--excel_txt', type=str, default='stage2_test_metrics_live.xlsx',
                    help='Excel file for live PSNR/SSIM logging')
...
# Network definitions
# ----------------------------
MyEnsembleNet = fusion_net_histoformer2().to(device)
...
MyEnsembleNet.load_state_dict(stage1_state, strict=True)
print('--- MyEnsembleNet loaded ---')

model2 = AdaIR().to(device)
print('AdaIR parameters:', sum(param.numel() for param in model2.parameters()))

# Freeze stage-1 model
MyEnsembleNet.eval()
for p in MyEnsembleNet.parameters():
    p.requires_grad = False

# --- optimizer --- #
G_optimizer = torch.optim.Adam(model2.parameters(), lr=learning_rate)
scheduler_G = torch.optim.lr_scheduler.MultiStepLR(G_optimizer, milestones=[200, 300, 400], gamma=0.5)
...
# Evaluation
# ----------------------------
def evaluate_test_metrics(stage1_model, refine_model, dataloader, device, pad_multiple=32):
    stage1_model.eval()
    refine_model.eval()
    ...
    with torch.no_grad():
        for hazy, clean in tqdm(dataloader, desc='Testing', leave=False):
            ...
            mid = stage1_model(hazy_pad)
            output = refine_model(mid)
...
for epoch in range(0, train_epoch):
    ...
    for batch_idx, (hazy, clean) in enumerate(pbar):
        ...
        total_loss = smooth_loss_l1 + 0.01 * perceptual_loss + 0.2 * msssim_loss_
    ...
    avg_psnr, avg_ssim = evaluate_test_metrics(
        MyEnsembleNet,
        model2,
        test_loader,
        device,
