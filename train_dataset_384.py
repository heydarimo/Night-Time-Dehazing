...
class dehaze_train_dataset(Dataset):
    def __init__(self, train_dir):
        ...
        self.list_train=[]
        for line in open(os.path.join(train_dir, 'train_train.txt')):  
            line = line.strip('\n')    
            if line!='':
                self.list_train.append(line)    
        #'./NTIRE2021_Train_Hazy/'
        self.root_hazy=os.path.join(train_dir, 'hazy/')       
        self.root_clean =os.path.join(train_dir, 'gt/')
    ...
    def __getitem__(self, index, is_train = True):
        if is_train:
            ...
            hazy_img = np.array(self.root_hazy + self.list_train[index])
            gt_img = np.array(self.root_clean + self.list_train[index])
            hazy_shape = hazy_img.shape
            gt_shape = gt_img.shape

            assert hazy_shape == gt_shape
