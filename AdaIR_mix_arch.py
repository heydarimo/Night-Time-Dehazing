...
class AdaIR(nn.Module):
    def __init__(self, 
        ...
        self.illu_std = illu_std
        self.ill_level = ill_level
        ...
        self.mix.append(Mix(m=-0.8))
        self.mix.append(Mix(m=-0.6))

        self.illu = torch.nn.Parameter(torch.FloatTensor([1.0]), requires_grad=True)
    ...
    def forward(self, inp_img):
        ...
        out_enc_level2 = self.encoder_level2(inp_enc_level2) # dim*2

        inp_enc_level3 = self.down2_3(out_enc_level2) #dim*4

        out_enc_level3 = self.encoder_level3(inp_enc_level3)  #dim*4

        inp_enc_level4 = self.down3_4(out_enc_level3) #dim*8       
        latent = self.latent(inp_enc_level4)   #dim*8

        if self.decoder:
            latent = self.fre1(inp_img, latent)
      
        inp_dec_level3 = self.up4_3(latent)  #dim*4

        inp_dec_level3 = self.mix[2](out_enc_level3, inp_dec_level3) #dim*4

        #inp_dec_level3 = torch.cat([inp_dec_level3, out_enc_level3], 1)
        #inp_dec_level3 = self.reduce_chan_level3(inp_dec_level3)

        out_dec_level3 = self.decoder_level3(inp_dec_level3)  #dim*4
        ...
        inp_dec_level2 = self.mix[1](out_enc_level2, inp_dec_level2)  #dim*2

        #inp_dec_level2 = torch.cat([inp_dec_level2, out_enc_level2], 1)
        #inp_dec_level2 = self.reduce_chan_level2(inp_dec_level2)

        out_dec_level2 = self.decoder_level2(inp_dec_level2)  #dim*2

        if self.decoder:
            out_dec_level2 = self.fre3(inp_img, out_dec_level2)

        inp_dec_level1 = self.up2_1(out_dec_level2)   #dim*1
        inp_dec_level1 = self.mix[0](out_enc_level1, inp_dec_level1)  #dim*2

        #inp_dec_level1 = torch.cat([inp_dec_level1, out_enc_level1], 1)
        
        out_dec_level1 = self.decoder_level1(inp_dec_level1)  # change dim*2 to dim

        out_dec_level1 = self.refinement(out_dec_level1) ## change dim*2 to dim

        out_dec_level1 = self.output(out_dec_level1) + inp_img

        # Only for bright outputs with low std
        if out_dec_level1.mean() > self.ill_level and out_dec_level1.std() < self.illu_std: 
            out_dec_level1 = out_dec_level1 * self.illu
