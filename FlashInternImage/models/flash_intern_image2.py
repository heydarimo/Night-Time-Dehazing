...
class FlashInternImage(nn.Module):
    ...
    def forward(self, x):
        ...
        x1, shape1 = self.levels[0](x, shape=shape0) 
        h, w = shape1
        x1_feat = x1.view(N, h, w, -1)
        x1_feat = x1_feat.permute(0, 3, 1, 2)



        x2, shape2 = self.levels[1](x1, shape=shape1)
        h, w = shape2
        x2_feat = x2.view(N, h, w, -1)
        x2_feat = x2_feat.permute(0, 3, 1, 2)

        x3, shape3 = self.levels[2](x2, shape=shape2)
        h, w = shape3
        x3_feat = x3.view(N, h, w, -1)
        x3_feat = x3_feat.permute(0, 3, 1, 2)
