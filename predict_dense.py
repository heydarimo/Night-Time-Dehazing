...
class Stage2FlatModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.stage1 = fusion_net_histoformer2()
        self.refine = AdaIR()
...
def forward_tile_simple(model, x, tile=384, overlap=96, pad_mode="reflect"):
    ...
    for top in range(0, H, stride):
        for left in range(0, W, stride):
            ...
            if out is None:
                out = torch.zeros((1, pred.shape[1], H, W), device=x.device, dtype=x.dtype)
...
def load_flat_model(model, ckpt_path: str):
    ...
    if not isinstance(sd, dict):
        raise RuntimeError(f"Checkpoint format not supported: {ckpt_path}")
    ...
    print(f"[INFO] Loaded flat checkpoint from: {ckpt_path}")


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--data_root", required=True, help="Path containing hazy/ and gt/")
    ap.add_argument("--ckpt", required=True, help="Flat merged checkpoint")
    ...
    model = Stage2FlatModel().to(device)
    load_flat_model(model, args.ckpt)
    model.eval()
